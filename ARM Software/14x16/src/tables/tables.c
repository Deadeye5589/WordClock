...
tables_fill_words (uint8_t * words, uint_fast8_t hh, uint_fast8_t mm)
{
    ...
    if (tables.complete)
    {
        ...
        //Specific code for 14x16 word clock
        if((mm == 15) || (mm == 30) || (mm == 45))                  //Handle this clock specific problem with halb, viertel vor oder viertel nach
        {
            if(hh < 12)                                             //used 2nd half of 24h table for hour position 2
                hh += 12;
        ...
        {
            if(hh >= 12)                                            //while in normal cases we use the first half of the 12 hours mapped to 24
                hh -= 12;
            word_idx_p = tables.hours[hh];                              // get the hour words from hour table
        }
        //End of specific code for 14x16 word clock
        ...
        for (idx = 0; idx < MAX_HOUR_WORDS && word_idx_p[idx] != 0; idx++)
        {
            ...
            // Added && !((mm == 15) || (mm == 30) || (mm == 45) to handle 14x16 clock
            else if (word_idx_p[idx] == WP_IF_HOUR_IS_0 && !((mm == 15) || (mm == 30) || (mm == 45)))            // if hour is null take word index + 1, otherwise word index + 2
