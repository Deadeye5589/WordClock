 ...
 *-------------------------------------------------------------------------------------------------------------------------------------------
 */
#if defined (BLACK_BOARD)                       // TFT & SSD1963 only for STM32F407VE
...
static uint_fast8_t ssd1963_is_up = 0;
...
ssd1963_read_flags_from_eep (void)
{
    ...
    {
        flags &= SSD1963_GLOBAL_FLAGS_MASK;

        if (ssd1963.flags != flags)
        {
            ssd1963.flags = flags;
 ...
 */
void
ssd1963_set_flags (uint_fast8_t new_flags, uint_fast8_t do_sync)
{
