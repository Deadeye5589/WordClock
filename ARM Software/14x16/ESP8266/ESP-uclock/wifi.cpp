 ...
 */
int
wifi_connect(const char * ssid, const char * key, bool do_disconnect)
{
    int cnt;
    int connected = false;
    ...
    if (WiFi.status() != WL_CONNECTED)
    {
        ...
        {                                                                             // in case of a timeout we have an empty ssid
            ...
            {                                                                         // connection successful
                wifi_connect (WiFi.SSID().c_str(), WiFi.psk().c_str(), false);        // reconnect now to make ssid & key permanent
            }
        }
    }
}
