...
#define STM32_BOOT0_PIN1                    4           // fm: changed from 16 to 4 in Version 2.6.0
#define STM32_BOOT0_PIN2                    5           // fm: Bug in Shield v3: Shields use GPIO5
#define STM32_RESET_PIN                     14
...
stm32_flash_image (bool do_flash)
{
    ...
    File f = LittleFS.open("stm32.hex", "r");
                    ...
                    if (rectype == 0)                               // Data Record
                    {
                        for (dri = 0; dri < datalen; dri++)
                        {
                            ...
                            if (! do_flash) // only check
                            {
                                ...
                                {
                                    sprintf (logbuf, "warning: gap in line %d\r\n", line);
                                    http_send (logbuf);
                                }
...
stm32_activate_bootloader (void)
{
    ...
    delay (200);
    Serial.begin (115200, SERIAL_8E1);                      // switch to 8 bit even parity
    delay (200);
    Serial.swap ();                                         // swap UART pins: GPIO15 is now TX, GPIO13 is now RX
...
void
stm32_flash_init (void)
{
    pinMode(STM32_BOOT0_PIN1, OUTPUT);                      // BOOT0 GPIO4
    digitalWrite(STM32_BOOT0_PIN1, LOW);
    pinMode(STM32_BOOT0_PIN2, OUTPUT);                      // BOOT0 GPIO5
    digitalWrite(STM32_BOOT0_PIN2, LOW);
    pinMode(STM32_RESET_PIN, INPUT);
    pinMode(13, INPUT);     // swapped RX, default is input
    pinMode(15, INPUT);     // swapped TX, default is input
}
