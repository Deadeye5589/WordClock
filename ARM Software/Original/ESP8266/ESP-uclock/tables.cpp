...
#define ILLUMINATION_FLAG_IT_IS 0x80                    // highest bit used as flag: word is member of "IT IS"
...
checkfile (const char * filename)
{
    ...
    {
        local_version = readhex (fp);
        local_wc_rows = readhex (fp);
    }
    else
    {
        local_version = 0;
        local_wc_rows = version_magic;
    }
    ...
    for (i = 0; i < local_wp_count; i++)
    {
        ...
        local_len = readhex(fp) & ILLUMINATION_LEN_MASK;
    ...
    if (local_version == 0)
    {
        local_tbl_it_is[0] = readhex (fp);                                    // old: tbl_it_is[0]
    ...
    if (local_version == 0)
    {
        if (local_tbl_it_is[0] > 0)
        {
            local_max_hour_words++;
        }

        if (local_tbl_it_is[1] > 0)
        {
            local_max_hour_words++;
        }
    }

    if (local_max_hour_words > MAX_HOUR_WORDS)
    {
        fp.close ();
        Serial.println ("- error: max_hour_words > MAX_HOUR_WORDS");
        return 0;
    }
    ...
    for (i = 0; i < local_hour_modes_count; i++)
    {
        for (j = 0; j < local_hour_count; j++)
        {
            ...
            for (k = local_h_offset; k < local_max_hour_words; k++)
            {
                ch = readhex(fp);
    ...
    if (local_max_minute_words > MAX_MINUTE_WORDS)
    {
        fp.close ();
        Serial.println ("- error: max_minute_words > MAX_MINUTE_WORDS");
        return 0;
    }
...
readfile (const char * filename)
{
    ...
    {
        tbl_it_is[0] = TABLES_VERSION_MAGIC;                            // new: store magic (0xFF) here
        tbl_it_is[1] = version;                                            // new: store version here
    }
